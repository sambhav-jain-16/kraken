...
func (c CAStoreConfig) Validate() error {
	if c.MemoryCache.Enabled && c.MemoryCache.TTL == 0 {
		return fmt.Errorf("memory_cache_ttl must be specified when memory_cache_enabled is true")
	}
	return nil
}
